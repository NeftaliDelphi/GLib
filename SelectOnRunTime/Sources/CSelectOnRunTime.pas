...
Copyright (©) 2005,  by Germán Estévez (Neftalí)
  ...
  en el IDE de Delphi. Además se pueden mover y redimensionar los controles
  seleccionados
  ...
  Se pueden distinguir qué controles se seleccionan utilizando la propiedad
...
------------------------------------------------------------------------
  ...
  11/11/2020
    * Add eents OnBeginMove/OnEndMove
  26/10/2010 (v.1.5b)
    * (Thanks to Bill) Corrected bug on positioning Selection panels on
      runtime creted components.

  20/06/2007  (v.1.4)
    * Corregidos problemas con controles que están alineados alClient, AlTop,...
    * Modificada la posición donde se colocan las marcas.
  10/09/2006
	  * Corregidos problemas al mover componentes que están colocados dentro
		  de otros.
	  * Corregido bug al pintar las marcas de selección cuando un control está
		  dentro de otro.
	  * Bugs varios.
  24/03/2006
    * Añadidas propiedades nuevas para definir qué controles se seleccionan o no.
    * SelectTaggedControls y SelectTag (thks. Jose)
    * Corregido buq en la propiedad Selected cuando se asignaba por código.
    * En diseño las marcas no deben ser visibles.
  17/03/2006
    * Añadido parémetro CanMoveOutParent a los eventos de movimmiento para
  ...
  02/11/2005
    ...
    * Propiedad de color.
  03/10/2005
    * Implementado el movimiento de los controles y de las marcas de selección.
  30/09/2005
    * Creación.
...
type
  ...
  {: Evento para el iniciao del movimiento de controles}
  TOnBeginEndMoveEvent = procedure (Sender: TObject; Control:TControl;
  ...
  private
    _InReposition: boolean;       // Se está recolocando un control
    _NodePositioning: Boolean;    // Si se está recolocando una marca
    ...
    _CapturedEvents:Boolean;            // Si están capturados ya los eventos del control
    ...
    FSelectTag: Integer;
    FOnBeginMove: TOnBeginEndMoveEvent;
    FOnEndMove: TOnBeginEndMoveEvent;
    ...
    //: Crea los nodos que visalizan la selección.
  ...
  published
    ...
    //: Activa que sólo se seleccionen los controles con un determinado TAG
    ...
    property OnBeginMove:TOnBeginEndMoveEvent read FOnBeginMove write FOnBeginMove;
    property OnEndMove:TOnBeginEndMoveEvent read FOnEndMove write FOnEndMove;
...
begin
  ...
  if (Self.FActive) AND (Sender is TControl) then begin
    ...
    // Lanzar el evento inicial
    if Assigned(FOnBeginMove) then
      OnBeginMove(Self, TControl(Sender), TControl(Sender).Left, TControl(Sender).Top);
  end;
end;
...
begin
  ...
  if (Self._InReposition) then begin
    ...
    if Assigned(FOnEndMove) then
      OnEndMove(Self, TControl(Sender), TControl(Sender).Left, TControl(Sender).Top);
  end;
...
//: Método de acceso apare escritura a la prop. active.
procedure TSelectOnRunTime.NodeMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  // Está activo y el Sender es correcto
  if (Self.FActive) AND (Sender is TControl) then begin
    // Reposicionando nodo
...
var
  ...
  _CanResize:Boolean;
begin
  // Se está reposicionando un nodo?
...
//: Método de escritura de la propiedad SelectControl.
...
begin
  ...
  if not Assigned(Value) then begin
    Self.FSelectControl.Cursor := crDefault;
    // Están capturados los eventos y activo
